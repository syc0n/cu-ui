    ...
    function Update() {
        if (cu.HasAPI()) {
            $perfHud.html(
                ...
                "plUpd " + (cuAPI.netstats_players_updateBits / 8).toFixed(1) + " B / " + Math.round(cuAPI.netstats_players_udpateCount) + " E (" + (cuAPI.netstats_players_updateBits / cuAPI.netstats_players_udpateCount).toFixed(1) + " b/E)"
                );
        }
    }
